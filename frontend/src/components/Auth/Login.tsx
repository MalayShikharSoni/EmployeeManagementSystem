  ...
  return (
    <div className="containerr flex items-center justify-center h-[80vh] bg-[#cec0ad]">
      <div className="border-[3px] rounded-se-[75px] rounded-es-[75px] rounded-ee-[75px] p-20 border-[#ad9676] bg-[#cec0ad] max-sm:m-[10px]">
        <form onSubmit={SubmitHandler} className="flex flex-col justify-center items-center w-full bg-transparent">
          <Link to={"/"}><div className="absolute top-7 left-7 bg-transparent max-sm:top-10 max-sm:left-5">
            <img src={BackButton} alt="BackButton" className="bg-transparent w-auto h-[65px]" /></div></Link>
          {error && (<div className="w-full mb-4 p-3 bg-red-100 border border-red-400 text-red-700 rounded-se-[15px] rounded-es-[15px] rounded-ee-[15px] text-center">{error}</div>)}
          ...
          <div className="relative mt-[70px] w-[85%] bg-[#ad9676] h-[1px]">
            <Link to="/signup"><div className="absolute w-[55%] text-center text-[13px] text-[#ad9676] font-semibold left-1/2 -translate-x-1/2 -translate-y-1/2 bg-[#cec0ad]">
