  ...
  return (
    <div className="containerr flex items-center justify-center h-[80vh] bg-[#cec0ad]">
      <div className="border-[3px] rounded-se-[75px] rounded-es-[75px] rounded-ee-[75px] p-20 border-[#ad9676] bg-[#cec0ad] max-sm:m-[10px] max-sm:p-[50px]">
        <form onSubmit={handleSignup} className="flex flex-col justify-center items-center w-full bg-transparent">
          <Link to={"/"}><div className="absolute top-7 left-7 bg-transparent">
            <img src={BackButton} alt="BackButton" className="bg-transparent w-auto h-[65px]" /></div></Link>
          {error && (<div className="w-full mb-4 p-3 bg-red-100 border border-red-400 text-red-700 rounded-se-[15px] rounded-es-[15px] rounded-ee-[15px] text-center text-sm">{error}</div>)}
          <div className="flex flex-row gap-4 mb-4 w-full bg-transparent max-sm:gap-2">
            <button type="button" onClick={() => setRole("employee")} disabled={isLoading}
              className={`flex-1 flex flex-col items-center justify-center gap-2 p-4 rounded-se-[20px] rounded-es-[20px] rounded-ee-[20px] border-[3px] transition-all duration-300 cursor-pointer
                ${role === "employee" ? "border-[#8b6c3e] bg-[#ad9676] shadow-lg scale-[1.03]" : "border-[#ad9676] bg-[#cec0ad] hover:border-[#9c815a] hover:bg-[#c4b49e]"}
              ...
              <span className={`font-bold text-[16px] bg-transparent transition-colors duration-300 max-sm:text-[13px] ${role === "employee" ? "text-[#cec0ad]" : "text-[#9c815a]"}`}>Employee</span>
            </button>
            <button type="button" onClick={() => setRole("admin")} disabled={isLoading}
              className={`flex-1 flex flex-col items-center justify-center gap-2 p-4 rounded-se-[20px] rounded-es-[20px] rounded-ee-[20px] border-[3px] transition-all duration-300 cursor-pointer
                ${role === "admin" ? "border-[#8b6c3e] bg-[#ad9676] shadow-lg scale-[1.03]" : "border-[#ad9676] bg-[#cec0ad] hover:border-[#9c815a] hover:bg-[#c4b49e]"}
                disabled:opacity-50 disabled:cursor-not-allowed max-sm:p-3`}>
              ...
              <span className={`font-bold text-[16px] bg-transparent transition-colors duration-300 max-sm:text-[13px] ${role === "admin" ? "text-[#cec0ad]" : "text-[#9c815a]"}`}>Admin</span>
            </button>
          </div>
          ...
          <div className="relative mt-[70px] w-[85%] bg-[#ad9676] h-[1px]">
            <Link to={"/login"}><div className="absolute w-[55%] text-center text-[13px] text-[#ad9676] font-semibold left-1/2 -translate-x-1/2 -translate-y-1/2 bg-[#cec0ad]">
