...
const TaskList = memo(({ tasks = [], taskCounts = {}, refreshTasks, data }) => {
  ...
  useGSAP(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      tl1.to(".popupBubble3", {
        duration: 0.6, scale: 1.2, yoyo: true, repeat: -1,
      });
      tl1.to(".popupBubble2", {
        duration: 0.6, scale: 1.2, delay: 0.3, yoyo: true, repeat: -1,
      });
      tl1.to(".popupBubble1", {
        duration: 0.6, scale: 1.2, delay: 0.3, yoyo: true, repeat: -1,
      });
      tl2.to(".popupBubble", {
        duration: 3, opacity: 0, repeat: -1, yoyo: true, delay: 4.2,
      });
      tl1.to(".popupText", {
        duration: 3, delay: 2, opacity: 1, repeat: -1, yoyo: true,
      });
  ...
  const TaskCountBubble = ({ count, label }) => (
  ...
  return (
    <div ref={containerRef} className="bg-transparent ml-[3vw]">
      ...
      <TaskRow>
        {newTasks.length > 0 ? (
          newTasks.map((task) => (
            <NewTask key={task.id} data={task} refreshTasks={refreshTasks} />
          ))
        ) : (
          <EmptyState />
        )}
      ...
      <TaskRow>
        {activeTasks.length > 0 ? (
          activeTasks.map((task) => (
            <AcceptedTask key={task.id} data={task} refreshTasks={refreshTasks} />
          ))
        ) : (
          <EmptyState />
        )}
      ...
      <TaskRow>
        {completedTasks.length > 0 ? (
          completedTasks.map((task) => (
            <CompletedTask key={task.id} data={task} />
          ))
        ) : (
          <EmptyState />
        )}
